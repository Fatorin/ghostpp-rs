...
impl GameHost {
    pub fn new(config: &Config) -> Result<Self, io::Error> {
        let bind_address = config.get_string("bot_bindaddress").unwrap_or_default();
        let host_port = get_u16_from_config(config, "bot_hostport", 6112);
        let udp_broadcasttarget = config.get_string("udp_broadcasttarget").unwrap_or_default();
        ...
        Ok(Self {
            ...
            map_cfgpath: config.get_string("bot_mapcfgpath").unwrap_or_default(),
    ...
    // legacy reference skeleton: mirrors the C++ CGHost::CreateGame, so the argument count is deliberate
