...
**經典魔獸爭霸 III 主機機器人 [GHost++](https://github.com/uakfdotb/ghostpp) 的 Rust 重寫版。**
...
ghostpp-rs 的目標平台為 **Warcraft III 1.26~1.28 + PVPGN 伺服器**(如私服/對戰平台)。以 tokio 非同步 actor 架構全面取代原版 C++ 的單執行緒 50ms select 輪詢,協定層與 C++ 原版逐 byte 對照移植。
...
每個 release 都會在 [Releases](https://github.com/Fatorin/ghostpp-rs/releases) 頁面發布預先建置好的執行檔:

| 平台 | 檔案 | 說明 |
|---|---|---|
| Windows x64 | `ghostpp-rs-windows-x64.exe` | |
| macOS(Apple Silicon) | `ghostpp-rs-macos-arm64` | |
| Linux x64 | `ghostpp-rs-linux-x64` | glibc 發行版(Debian、Ubuntu、Fedora…) |
| Linux ARM64 | `ghostpp-rs-linux-arm64-musl` | 全靜態 — 任何 ARM64 發行版皆可執行,含 Alpine |

## 從原始碼建置

上列四個平台皆由 CI 建置並驗證。

需求:Rust(stable)、CMake ≥ 4.1(stormlib-sys 用 CMake 編譯內含的 StormLib C++ 原始碼),以及各平台的 C/C++ 工具鏈:

- **Windows** — MSVC 建置工具。若使用 VS2026 工具鏈,需先執行 `cargo update -p cmake`(cmake crate 舊版與 VS2026 不相容)。
- **Linux** — C/C++ 編譯工具鏈與 zlib / bzip2 開發套件(StormLib 連結用)。
  ...
  ```
- **macOS** — Xcode Command Line Tools 與 CMake(`brew install cmake`);zlib 與 bzip2 由 SDK 內建提供。已由 CI 在 Apple Silicon 上驗證。
...
啟動後 bot 會登入 PVPGN 並(若啟用 autohost)自動開房;或用密語 `!pub <房名>` 手動建房。指令用法見 [COMMANDS.zh-TW.md](COMMANDS.zh-TW.md)。
...
本專案為 [GHost++](https://github.com/uakfdotb/ghostpp)(Trevor Hogan 原作)的重寫,協定知識與行為語意皆源自原專案,依 [Apache License 2.0](LICENSE) 授權。
